...
- [x] **Deep-path (CFR-D Phase 5) engineering tail — 2026-07-27, $0 local.** Four deferred items from the 2026-07-23 0×0 probe, all gated by bit-identical-certificate tests: (1) the composed artifact STREAMS to disk subgame by subgame instead of being returned as a whole-profile map and re-enumerated against a freshly built full arena — 4.35× lower peak RSS at 8×8/2,000 deals, output content-identical (`compare-policies` max TV 0.000000 over 3.87M rows), and it removed the per-subgame key→local maps entirely (~30 GB at 0×0, their only consumer was composition); (2) `--arena-cache DIR` packs each subgame arena on first build and mmaps it thereafter, DEFAULT ON at `<checkpoint>.arenas` — 3.3–3.7× wall per round at jobs=1 for 0.4–7% peak RSS, versus 2.0–2.3× RSS for `--keep-arenas`; (3) `--resume` may RAISE `--rounds`, and an extension is bit-identical to having asked for the longer run up front (warm-up re-anchored, CBV tail window cleared at the anchor) — the mechanism the "ε=0.01 now, 2.5e-4 later at zero waste" strategy assumed; (4) `--cert-jobs` (default `min(jobs, 8)`) bounds the memory-critical certificate pool alone. `SOLVER_BENCHMARKS.md` 2026-07-27; plan 84's remaining items are spend, not code.
